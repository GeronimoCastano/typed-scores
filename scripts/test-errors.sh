...
check_error tests/errors/invalid-theme.typ "unsupported notation theme"
check_error tests/errors/invalid-auto-theme-fill.typ "auto cannot follow a non-color text fill"
...
check_error tests/errors/turn-fingering-without-turn.typ "requires turn, chromatic-turn, or inverted-turn"
check_error tests/errors/multiple-ornaments.typ "only one ornament"
...
check_error tests/errors/invalid-lyrics-type.typ "lyrics must be a string or an array of verse strings"
check_error tests/errors/unknown-lyrics-staff.typ "lyrics reference an unknown staff"
check_error tests/errors/lyrics-count-mismatch.typ "lyrics do not account for every pitched event"
check_error tests/errors/invalid-lyric-control.typ "hyphen control must immediately follow a syllable"
check_error tests/errors/unopened-melisma.typ "melisma extender has no preceding syllable"
check_error tests/errors/dangling-lyric-hyphen.typ "hyphenated syllable has no following syllable"
check_error tests/errors/invalid-lyric-size.typ "lyric-size: value must be a positive number"
check_error tests/errors/invalid-lyric-font.typ "score lyric-font: value must be a non-empty font family string or none"
check_error tests/errors/invalid-lyric-gap.typ "lyric-gap: value must be a non-negative number"
check_error tests/errors/invalid-verse-gap.typ "verse-gap: value must be a positive number"
check_error tests/errors/impossible-lyric-width.typ "no legal system partition fits the requested width"
check_error tests/errors/overlapping-verse-gap.typ "verse baselines are closer than the lyric text size"
check_error tests/errors/unknown-switch-staff.typ "staff switch @middle names an unknown staff"
check_error tests/errors/redundant-staff-switch.typ "staff switch @upper does not change staff"
check_error tests/errors/trailing-staff-switch.typ "staff switch @lower must be followed by a note, chord, rest, or spacer"
check_error tests/errors/staff-switch-in-tremolo.typ "staff switches are not supported inside alternating tremolos"
check_error tests/errors/annotated-spacer.typ "spacer 's' draws nothing and cannot carry annotations"
check_error tests/errors/crossed-split-chord.typ "places C4 on staff upper below E4 on staff lower"
check_error tests/errors/empty-split-chord-side.typ "must list at least one pitch after @lower"
check_error tests/errors/distant-split-chord.typ "which are not adjacent staves"
check_error tests/errors/cross-staff-tie.typ "tie after C4 moves to another staff"
check_error tests/errors/tie-into-reset-staff.typ "tie after G3 moves to another staff"
check_error tests/errors/distant-staff-beam.typ "a beam group joins notes on staves that are not adjacent"
